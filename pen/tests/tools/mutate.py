...
MUTATIONS: list[tuple[str, str, str, str, str]] = [
    ...
    (
        "v0.10.6 cap=0 就是不限（默认档逐字节一致的唯一实现点）",
        "pen/meter.py",
        "    return cap > 0 and (spent + max(0, headroom)) >= cap",
        "    return (spent + max(0, headroom)) >= cap",
        "zero_is_byte_for_byte_identical or cross_book_token_gate_is_off",
    ),
    (
        "v0.10.6 每轮上限要给收口那一枪留余量",
        "pen/tutor.py",
    ...
    (
        ...
        "            if False:",
        "stops_before_the_second_call",
    ),
    (
        "v0.10.6 深挖被预算拦下时不许丢掉第一枪的产出",
        "pen/probe.py",
    ...
    (
        "v0.10.6 跨书 token 闸是第三道，不是替换前两道",
        "pen/agent/tools_impl.py",
